...
ARG KCOV_VERSION=v40
ARG KCOV_SOURCE=https://github.com/SimonKagstrom/kcov/releases/download/${KCOV_VERSION}/kcov-amd64.tar.gz

RUN dnf install -y epel-release && \
      dnf install -y --enablerepo "$ENABLED_REPO" xz git make doxygen npm wget libxml2 binutils diffutils yamllint findutils 'dnf-command(download)' && \
      ...
      rm -f shellcheck.tar && \
      ln -sf /usr/lib64/libopcodes-2.30-*.el*.so /usr/lib64/libopcodes-2.34-system.so && \
      ln -sf /usr/lib64/libbfd-2.30-*.el*.so /usr/lib64/libbfd-2.34-system.so && \
